...
public class NeuralNetwork {
    ...
    public void initWeightsBiases() {
        ...
        biases = new Neuron[dim][];
        for (int i = 0; i < dim; i++) {
            biases[i] = new Neuron[network[i + 1].length];
        }
        ...
        for (int i = 0; i < biases.length; i++) {
            for (int j = 0; j < biases[i].length; j++) {
                biases[i][j] = new Neuron(1, -1 + (2 * rand.nextDouble()));
            }
        }
    }
    ...
    public void backpropagation(double[] expectedoutput) {
       ...
       */
        ...
        newWeights = NeuralNetworkUtil.addMatrices(oldweights[oldweights.length - 1], out_delta_weights);
