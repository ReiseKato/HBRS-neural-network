...
public class NeuralNetworkUtilVic {
     ...
     */
    public static NeuronVic[] matrixVectorMultiplication(double[][] matrix, NeuronVic[] v) {
        NeuronVic[] res = new NeuronVic[matrix.length];// Ergebnis Vektor mit Länge = Anzahl der Zeilen der Matrix
     ...
     * @return String[]
     */
    public static String[] neuronToString(NeuronVic[] n) {
    ...
    public static String[] biasNeuronToString(NeuronVic[] n) {
     ...
     * @return Bias Neuron
     */
    public static NeuronVic[] stringToBiasNeuron(String[] st) {
        NeuronVic[] temp = new NeuronVic[st.length];
        //erzeuge Neuron[] mit Länge von Parameter
        for (int i = 0; i < temp.length; i++) {
            try {
                //versuche für jeden String ein Neuron zu erstellen
                temp[i] = new NeuronVic(1, Double.parseDouble(st[i]));
            } catch (NumberFormatException e) {
                e.printStackTrace();
            }
